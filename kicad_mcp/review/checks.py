...
_POWER_PATTERNS = [
    ...
    r"^V?BAT", r"BATTERY", r"^VIN", r"^VM$", r"^VBUS",
    r"^VCC", r"^VDD", r"^VOUT", r"^\+?3V3", r"^\+?5V", r"^\+?12V", r"^\+?1V",
    r"MOTOR", r"^OUT\d", r"^SW\d?$", r"PACK", r"^P\+", r"^P-",
]
...
def ipc2221_capacity_a(width_mm: float, dT_c: float = 10.0, copper_oz: float = 1.0,
...
def ipc2221_width_mm(current_a: float, dT_c: float = 10.0, copper_oz: float = 1.0,
                     external: bool = True) -> float:
...
def check_erc(erc: dict) -> list[Finding]:
    ...
    for vtype, items in sorted(by_type.items(), key=lambda kv: -len(kv[1])):
        ...
        findings.append(Finding(
            id=f"erc-{vtype}", severity=sev, domain=Domain.ELECTRICAL,
            title=f"ERC: {vtype} ×{len(items)}",
            detail=f"{sample}" + (f"  (e.g. {loc.get('at')})" if loc.get("at") else ""),
            recommendation=rec, evidence=f"kicad-cli sch erc (json), {len(items)} occurrences",
            check="erc",
        ))
    return findings
...
def check_erc_suppressions(pro: ProjectSettings | None, erc: dict) -> list[Finding]:
    ...
    if ignored:
        ...
        names = ", ".join(keys)
        findings.append(Finding(
            id="erc-ignored", severity=Severity.MINOR, domain=Domain.HYGIENE,
            ...
            evidence="erc.json ignored_checks", check="erc_suppressions",
        ))
    at_warning = [k for k, v in downgraded.items() if v == "warning"]
    if len(at_warning) >= 5:
        findings.append(Finding(
            id="erc-downgraded", severity=Severity.NIT, domain=Domain.HYGIENE,
            ...
            evidence=".kicad_pro erc.rule_severities", check="erc_suppressions",
        ))
    return findings
...
def check_drc(drc: dict) -> list[Finding]:
    ...
    for vtype, items in sorted(by_type.items(), key=lambda kv: -len(kv[1])):
        ...
        sev = Severity.MAJOR if sev_raw == "error" else Severity.MINOR
        findings.append(Finding(
            id=f"drc-{vtype}", severity=sev, domain=Domain.DFM,
            title=f"DRC: {vtype} ×{len(items)}",
            detail=items[0].get("description", ""),
            recommendation="Resolve in Pcbnew's DRC panel.",
            evidence=f"kicad-cli pcb drc (json), {len(items)} occurrences", check="drc",
        ))

    if unconnected:
        findings.append(Finding(
            id="drc-unconnected", severity=Severity.BLOCKER, domain=Domain.ELECTRICAL,
            ...
            evidence="drc.json unconnected_items", check="drc",
        ))
    ...
    if parity:
        ...
        findings.append(Finding(
            id="drc-parity", severity=Severity.MAJOR, domain=Domain.DFM,
            ...
            recommendation="Run 'Update PCB from Schematic' (F8) and reconcile every diff "
                           "before fabrication.",
            evidence="kicad-cli pcb drc --schematic-parity", check="drc_parity",
        ))
    return findings
...
def check_net_classes(pro: ProjectSettings | None) -> list[Finding]:
    ...
    if len(classes) <= 1:
        ...
        findings.append(Finding(
            id="netclass-single", severity=Severity.MAJOR, domain=Domain.POWER_THERMAL,
            ...
            evidence=".kicad_pro net_settings.classes", check="net_classes",
        ))
    return findings
...
def check_trace_currents(board: Board, current_specs: dict | None = None,
                         dT_c: float = 10.0, power_nets: set | None = None) -> list[Finding]:
    ...
    poured = {p for p in getattr(board, "poured_nets", set())}
        ...
        if spec:
            ...
            if w < need * 0.95:
                if is_poured:
                    ...
                    findings.append(Finding(
                        id=f"trace-{name}", severity=Severity.MINOR, domain=Domain.POWER_THERMAL,
                    ...
                    findings.append(Finding(
                        id=f"trace-{name}", severity=Severity.MAJOR, domain=Domain.POWER_THERMAL,
                        ...
                        location={"net": name},
                        evidence="IPC-2221 (.kicad_pcb track geometry)", check="trace_currents",
                    ))
        elif w <= 0.30 and not is_poured:
            # informational capacity for suspiciously thin, NON-poured power nets
            findings.append(Finding(
                id=f"trace-cap-{name}", severity=Severity.MINOR, domain=Domain.POWER_THERMAL,
...
def check_decoupling(netlist: Netlist, board: Board | None = None) -> list[Finding]:
    ...
    for ic in sorted(ic_power_nets):
        for net in sorted(ic_power_nets[ic]):
            ...
            if not caps_on_net:
                findings.append(Finding(
                    id=f"decap-missing-{ic}-{net}", severity=Severity.MAJOR,
                    ...
                    location={"refdes": ic, "net": net}, evidence="netlist pin types",
                    check="decoupling",
                ))
                ...
                if dists and min(dists) > 5.0:
                    findings.append(Finding(
                        id=f"decap-far-{ic}-{net}", severity=Severity.MINOR,
                        domain=Domain.POWER_THERMAL,
...
def check_bom(netlist: Netlist) -> list[Finding]:
    findings: list[Finding] = []
    missing_val = [c["ref"] for c in netlist.components
                   if str(c.get("value", "")).strip().lower() in _PLACEHOLDER_VALUES]
    if missing_val:
        findings.append(Finding(
            id="bom-missing-value", severity=Severity.MINOR, domain=Domain.BOM,
            ...
            evidence="netlist components", check="bom",
        ))
    return findings
